"""
Tests for Sprint 3 ToolAnnotations (src/annotations.py).
...
class TestClassificationCorrectness:
    ...
    def test_dry_run_is_readonly(self):
        """WP-specific: dry_run tools preview changes without applying."""
        assert TOOL_ANNOTATIONS["dry_run_bulk_update"]["readOnlyHint"] is True

    def test_wp7_bulk_update_is_idempotent(self):
        """WP-7 bulk_update is set-based (idempotent), not additive."""
