"""
Sprint 2 tests: Server Instructions + Tool Guide (Item #13)
...
class TestServerInstructions:
    ...
    def test_mentions_wp_hardening(self):
        assert "WP-1" in SERVER_INSTRUCTIONS or "ProjectSession" in SERVER_INSTRUCTIONS
...
class TestToolGuidePayload:
    ...
    def test_has_tool_categories(self):
        ...
        # Must have our WP-specific categories
        expected = {
            "project_management", "read_tasks", "write_tasks",
            "resources", "calendars", "wp_hardening", "server_meta",
        }
        assert expected.issubset(set(cats.keys())), \
            f"Missing categories: {expected - set(cats.keys())}"

    def test_wp_hardening_includes_session_tools(self):
        wp = _TOOL_GUIDE["tool_categories"]["wp_hardening"]
